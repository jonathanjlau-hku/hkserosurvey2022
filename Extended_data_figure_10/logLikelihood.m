...
%%% Purpose: Function to calculate log likelihood (see methods)
%%% This likelihood function also relaxes seroconversion rates amongst 
%%% previously infected donors and is used for generating the sensitivity 
%%% analysis presented under Extended Data Figure 10.

%%% This likelihood function is compatible with "main" script of
%%% main_estimate.m
...
ff = 0.75;  
%%% ff: Additional parameter incorporating assumption on seroconversion rates amongst previously infected donors
%%% Here ff is set at 75%, corresponding to a seroconversion rate of 75%
%%% amongst previously infected donors.
...
% Caclulate cumulative proxy for unvaccinated
for ii = 1:numel(sero{1})
    cum_proxy_unvax(ii) = sum(proxy_unvax(ii,1:donation_day{1}(ii)-1),2);
end

% Calculate age scaling factors for every study subject given age data
for ii =1:numel(age)
     age_scaling_factors{ii} = pchip(age_knots, age_knots_factors,age{ii});
end

% Calculate cumulative proxy for BNT vaccinated individuals
...
% Calculate cumulative proxy for CoronaVac vaccinated individuals
...
% Calculate probability of infection for each individual (see Methods)
for vax_idx=1:numel(sero)
    yy = sero{vax_idx};
    if (vax_idx==1)
        p_infection{vax_idx} = 1-exp(-(age_scaling_factors{vax_idx}.*total_FOI).*cum_proxy_unvax)'; % p_infection for infected
    elseif (vax_idx==2)
        p_infection{vax_idx} = 1-exp(-(age_scaling_factors{vax_idx}.*total_FOI).*cum_proxy_BNT)'; % p_infection for vaccinated
    elseif (vax_idx==3)
        p_infection{vax_idx} = 1-exp(-(age_scaling_factors{vax_idx}.*total_FOI).*cum_proxy_C)';
    end
    p_seropos{vax_idx} = p_infection{vax_idx}*p_sens(vax_idx)*ff+(1-p_infection{vax_idx}*ff)*(1-p_spec(vax_idx));
