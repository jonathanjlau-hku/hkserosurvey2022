...
%%%%%%% Assume no delay to VE taking effect %%%%%%%%%%
%%%%% Read data
dir = 'inference_inputs/';
no_vax = readtable([dir 'no_vax_input_data.csv']);  % data file corresponding to unvaccinated individuals, and so on          
B = readtable([dir 'b_input_data.csv']);    % data file corresponding to individuals with one homologous dose of BNT, and so on
BB = readtable([dir 'bb_input_data.csv']);                   
BBB = readtable([dir 'bbb_input_data.csv']);              
BBBB = readtable([dir 'bbbb_input_data.csv']);

S = readtable([dir 's_input_data.csv']);    % data file corresponding to individuals with one homologous dose of CoronaVac (Sinovac), and so on
...
% Vax_idx: Group 1 is unvacinated; Group 2 is vaccinated; Group 3 is boosted
vax_idx = 1;
sero{vax_idx} = no_vax.sero;      
age{vax_idx} = no_vax.age;
gender{vax_idx} = categorical(no_vax.gender);
donation_day{vax_idx} = no_vax.donation_day-t_start+1;

vax_idx = 2;
sero{vax_idx} = [B.sero; BB.sero; BBB.sero; BBBB.sero]; % seropositivity
age{vax_idx} = [B.age; BB.age; BBB.age; BBBB.age];      % participant age
gender{vax_idx} = [categorical(B.gender); categorical(BB.gender); categorical(BBB.gender); categorical(BBBB.gender)];   % participant gender (not used)
donation_day{vax_idx} = [B.donation_day; BB.donation_day; BBB.donation_day; BBBB.donation_day]-t_start+1;   % day when participant donated serum
...
dose_day{vax_idx,1} = [B.dose_1_day; BB.dose_1_day;BBB.dose_1_day;BBBB.dose_1_day];
dose_day{vax_idx,2} = [B.dose_2_day; BB.dose_2_day;BBB.dose_2_day;BBBB.dose_2_day];
dose_day{vax_idx,3} = [B.dose_3_day; BB.dose_3_day;BBB.dose_3_day;BBBB.dose_3_day];
dose_day{vax_idx,4} = [B.dose_4_day; BB.dose_4_day;BBB.dose_4_day;BBBB.dose_4_day];
...
dose_day{vax_idx,1} = [S.dose_1_day; SS.dose_1_day;SSS.dose_1_day;SSSS.dose_1_day];
dose_day{vax_idx,2} = [S.dose_2_day; SS.dose_2_day;SSS.dose_2_day;SSSS.dose_2_day];
dose_day{vax_idx,3} = [S.dose_3_day; SS.dose_3_day;SSS.dose_3_day;SSSS.dose_3_day];
dose_day{vax_idx,4} = [S.dose_4_day; SS.dose_4_day;SSS.dose_4_day;SSSS.dose_4_day];

% Set age PCHIP (piecewise cubic hermite interpolating polynomial) knots for age effect estimation
age_knots = [10 18 35 50 65];

% Obtain and process positive and negative controls data for sensitivity
...
%%%%%% Calculate matrix containing indicator variables by vaccine type and
%%%%%% dose for each study participant over time
...
chain.data = data;
chain.numStepsPerParameter = 3000;
chain.minNumStepsBeforeRestart = 1000;
chain.numStepsBetweenDisplay = 500;
chain.probAcceptRange = [0.2 0.7];
chain.startingPoint = [0.7 repmat(0.5,1,6) repmat(0.01,1,2) repmat(0.9,1,6) ones(1,4)];
chain.numParameters = numel(chain.startingPoint);
chain.UB = [1 repmat(1,1,6) repmat(1,1,2) repmat(1,1,6) repmat(10, 1, numel(age_knots)-1)];      % limit UB for waning rates to 0.2
chain.LB = zeros(1,chain.numParameters);
chain.totalNumSteps = chain.numStepsPerParameter*chain.numParameters;
chain.stepSTD = repmat(1, 1, chain.numParameters);
chain.logSpace = zeros(1, chain.numParameters);
chain.dir = 'Test';

 
chain = MCMC_Gibbs(@logLikelihood, @logPriorDist, chain);
disp(' ');

posterior = chain.record(chain.iter/3:2:end,:);
save('posterior_age.mat','posterior');

% Calculate initial VE for 2 and 3 doses of BNT or CoronaVac
posterior(:,2) = prod(posterior(:,2:4),2); 
posterior(:,3) = prod(posterior(:,3:4),2); 

posterior(:,5) = prod(posterior(:,5:7),2);
posterior(:,6) = prod(posterior(:,6:7),2); 
...
for var_idx = 1:numel(var_text)
    ...
    disp(prctile(posterior(:,var_idx), [50 2.5 97.5]));
end

% Reproduce age effect factors using PCHIP
age_range = min(age_knots):max(age_knots);

age_knots_factors = [prctile(posterior(:,16:17),[2.5;50;97.5]) ones(3,1) prctile(posterior(:,18:18+numel(age_knots)-4),[2.5; 50; 97.5])];

FOI = 1*pchip(age_knots,age_knots_factors,age_range);

% Calculate VE over time assuming exponential decay (see Methods)
tv = 0:200;

VE_BNT2_TS = repmat(posterior(:,2), 1, numel(tv)).*exp(-posterior(:,8)*tv);
VE_BNT3_TS = repmat(posterior(:,3), 1, numel(tv)).*exp(-posterior(:,8)*tv);
VE_BNT4_TS = repmat(posterior(:,4), 1, numel(tv)).*exp(-posterior(:,8)*tv);

VE_BNT_prctile{2} = prctile(VE_BNT2_TS, [2.5 25 50 75 97.5]);
VE_BNT_prctile{3} = prctile(VE_BNT3_TS, [2.5 25 50 75 97.5]);
VE_BNT_prctile{4} = prctile(VE_BNT4_TS, [2.5 25 50 75 97.5]);

VE_C2_TS = repmat(posterior(:,5), 1, numel(tv)).*exp(-posterior(:,9)*tv);
VE_C3_TS = repmat(posterior(:,6), 1, numel(tv)).*exp(-posterior(:,9)*tv);
VE_C4_TS = repmat(posterior(:,7), 1, numel(tv)).*exp(-posterior(:,9)*tv);

VE_C_prctile{2} = prctile(VE_C2_TS, [2.5 25 50 75 97.5]);
VE_C_prctile{3} = prctile(VE_C3_TS, [2.5 25 50 75 97.5]);
VE_C_prctile{4} = prctile(VE_C4_TS, [2.5 25 50 75 97.5]);
...
% Notwithstanding the 7 day delay to VE taking effect, the code below follows the same logic as above
vax_idx = 2;
...
%%%%%%%%%%%%%%%%%% Assume 14 day delay to VE taking effect %%%%%%%%%%%%%%%%%%%%%%
...
chain_14delay.probAcceptRange = [0.2 0.7];
...
chain_14delay.stepSTD = repmat(1, 1, chain_14delay.numParameters);
...
% Display posterior medians and 95% CI
var_text = {'IAR_unvax','VE_BNT2','VE_BNT3', 'VE_BNT4','VE_C2','VE_C3','VE_C4','waning_BNT','waning_C','sens_unvax','sens_BNT','sens_C','spec_unvax','spec_BNT','spec_C','10','18','50','65'};
for var_idx = 1:numel(var_text)
    disp(var_text{var_idx});
...
h = fill([age_range fliplr(age_range)], [dat(1,:) fliplr(dat(end,:))], [0.902 0.294 0.208]);
transparency = 0.1;
set(h, "FaceAlpha", transparency, "lineStyle", "none");

h_7delay = fill([age_range fliplr(age_range)], [dat_7delay(1,:) fliplr(dat_7delay(end,:))], [0.302 0.733 1]);
transparency = 0.1;
set(h_7delay, "FaceAlpha", transparency, "lineStyle", "none");

h_14delay = fill([age_range fliplr(age_range)], [dat_14delay(1,:) fliplr(dat_14delay(end,:))], "g");
transparency = 0.1;
set(h_14delay, "FaceAlpha", transparency, "lineStyle", "none");


plot(age_range, dat(round(size(dat,1)/2),:), "Color",[0.902 0.294 0.208]);
plot(age_range, dat_7delay(round(size(dat_7delay,1)/2),:),"Color",[0.302 0.733 1]);
plot(age_range, dat_14delay(round(size(dat_14delay,1)/2),:),"Color","g");

axis tight;
xlabel("Age");
ylabel("Relative FOI vs 35 year old subject");
legend("No delay","7 day delay","14 day delay");

save("Extended_data_Figure_1.mat",'age_range','dat','dat_7delay','dat_14delay');


%%% Randomly select 300 posterior samples for IAR and population immunity
%%% calculation
sampled_posteriors_dir = 'IAR_calculation/';
sampled_posteriors_7delay_dir = 'IAR_calculation_7delay/';
sampled_posteriors_14delay_dir = 'IAR_calculation_14delay/';

num_sampled_posteriors = randsample([1:size(posterior,1)],300);
sampled_posteriors = posterior(num_sampled_posteriors,:);

save([sampled_posteriors_dir 'sampled_posteriors_age.mat'],'sampled_posteriors')
...
%%% Summarise posterior results (median and 95% credible interval) on a
%%% xlsx spreadsheet (0 delay, 7 day delay and 14 day delay)

posterior_table = table(var_text',num2cell(prctile(posterior,50))',num2cell(prctile(posterior,2.5))',num2cell(prctile(posterior,97.5))');
posterior_7delay_table = table(var_text',num2cell(prctile(posterior_7delay,50))',num2cell(prctile(posterior_7delay,2.5))',num2cell(prctile(posterior_7delay,97.5))');
posterior_14delay_table = table(var_text',num2cell(prctile(posterior_14delay,50))',num2cell(prctile(posterior_14delay,2.5))',num2cell(prctile(posterior_14delay,97.5))');

filename = 'posteriors_BNT-C_age.xlsx';
writetable(posterior_table,filename,"Sheet",1,"Range","A1");
writetable(posterior_7delay_table,filename,"Sheet",1,"Range","G1");
writetable(posterior_14delay_table,filename,"Sheet",1,"Range","L1");

%%% Output VE over time for creating Figure 2
output_dir = 'Figure_2/VE/';
writematrix([tv; VE_BNT_prctile{2}]',[output_dir 'VE_BB_prctile_age.csv'])
writematrix([tv; VE_BNT_prctile{3}]',[output_dir 'VE_BBB_prctile_age.csv'])
writematrix([tv; VE_BNT_prctile{4}]',[output_dir 'VE_BBBB_prctile_age.csv'])

writematrix([tv; VE_C_prctile{2}]',[output_dir 'VE_CC_prctile_age.csv'])
writematrix([tv; VE_C_prctile{3}]',[output_dir 'VE_CCC_prctile_age.csv'])
writematrix([tv; VE_C_prctile{4}]',[output_dir 'VE_CCCC_prctile_age.csv'])
