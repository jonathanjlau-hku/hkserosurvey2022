...
% Purpose: Calculation IAR and population immunity assuming VE takes
% effect immediately
...
load("sampled_posteriors_age.mat");
% load("results_VE_delay_0.mat");
...
save ("no_delay_age.mat","proxy","-v7.3","-nocompression")
m = matfile("no_delay_age.mat","writable",true);
...
for ii = [2:-1:1 3:300]
    ...
    parfor jj = 1:individuals
         ...
         [jj_infect,jj_immunity] = infection_calc_vF(IAR_unvax,VE_BNT1,VE_BNT2,VE_BNT3,VE_BNT4,VE_C1,VE_C2,VE_C3,VE_C4,...
            waning_BNT, waning_C,age_scaling_factor(age(jj)+1),proxy,tv,tv_week,...
...
p_infection_weekly_cohort = zeros(7,6,proxyWeekLength);
immunity_weekly_cohort = zeros(7,6,proxyWeekLength);
...
% Calculate weekly IAR and population immunity (median and 95% CI) over the
% fifth wave from Jan 1, 2022 to July 31, 2022

parfor ii = 1:proxyWeekLength
    ii
    temp_p_infection_cohort = zeros(7,no_scenarios);
    temp_immunity_cohort = zeros(7,no_scenarios);
...
writematrix([reshape(p_infection_weekly_cohort(kk,:,:),[6 proxyWeekLength])' repmat(age_cohorts(kk),proxyWeekLength,1) repmat("No delay",proxyWeekLength,1) repmat("IAR",proxyWeekLength,1)],strcat("p_infection_weekly_no_delay_",age_cohorts(kk),".csv"));
writematrix([reshape(immunity_weekly_cohort(kk,:,:),[6 proxyWeekLength])' repmat(age_cohorts(kk),proxyWeekLength,1) repmat("No delay",proxyWeekLength,1) repmat("Pop immunity",proxyWeekLength,1)],strcat("immunity_weekly_no_delay_",age_cohorts(kk),".csv"));

end

writematrix([reshape(p_infection_weekly_all,[6 proxyWeekLength])' repmat("all",proxyWeekLength,1) repmat("No delay",proxyWeekLength,1) repmat("IAR",proxyWeekLength,1)] ,"p_infection_weekly_no_delay_all.csv")
writematrix([reshape(immunity_weekly_all,[6 proxyWeekLength])' repmat("all",proxyWeekLength,1) repmat("No delay",proxyWeekLength,1) repmat("Pop immunity",proxyWeekLength,1)],"immunity_weekly_no_delay_all.csv")
